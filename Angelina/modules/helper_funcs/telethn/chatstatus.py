from Angelina.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Angelina import SUDO_USERS
...
async def angelina_is_admin(chat_id: int):
    status = False
    angelina = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if angelina.id == user.id:
            status = True
            break
    return status
